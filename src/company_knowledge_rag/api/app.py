...
def create_app(
    ...
    def ready(request: Request) -> dict[str, str]:
        if not request.app.state.store.ready() or request.app.state.provider is None:
    ...
    def reindex_document(
        ...
        if document is None or not document.allowed_roles & principal.roles:
            raise HTTPException(status.HTTP_404_NOT_FOUND, "Document not found")
        source_path = settings.upload_dir / f"{document.id}{Path(document.source_name).suffix.lower()}"
...
def _build_qdrant_store(settings: Settings) -> QdrantChunkStore:
    embedder = GeminiEmbeddingProvider(settings.gemini_api_key, settings.embedding_model)
