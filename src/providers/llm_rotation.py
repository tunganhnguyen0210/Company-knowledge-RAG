...
try:
    ...
    _BaseRunnable = Runnable[Any, Any]
except ImportError:
    class _BaseRunnable:  # type: ignore[no-redef]
        pass
...
class GeminiRotatingRunnable(_BaseRunnable):
    ...
    def invoke(self, input: Any, config: Any = None, **kwargs: Any) -> Any:
        ...
        except (GeminiKeysExhausted, Exception) as exc:
            if not (isinstance(exc, GeminiKeysExhausted) or is_gemini_quota_error(exc) or is_gemini_transient_error(exc)):
                raise
            last_error = exc
