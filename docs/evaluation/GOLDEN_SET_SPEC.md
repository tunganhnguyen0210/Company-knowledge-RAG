...
| `gold_metadata` | `object` | Tọa độ pháp lý & Định danh tài liệu | Cấu trúc định vị vị trí câu hỏi trong tài liệu (`doc_id`, `part`, `chapter`, `section`, `article`, `clause`, `point`). |
...
Công cụ đánh giá tự động [`scripts/evaluate_golden_set.py`](file:///D:/User/ProjectGithub/hiepnguyenn-99/Company-knowledge-RAG/scripts/evaluate_golden_set.py) thực hiện đo lường chất lượng tập Golden Set dựa trên 3 nhóm chỉ số đa kịch bản (Type-Aware Metrics):
